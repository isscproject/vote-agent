...
class Xgb_Agent(BaseAgent):
    ...
    def act(self, obs, action_space):
        ...
        safe_actions = action_prune.get_filtered_actions(obs, self.prev_states)

        # if True :
        if near_target(my_position, items, enemies, dist, prev, 3) != None:
            bst = xgb.Booster(model_file='xgb.model')
            data = get_feature(board,safe_actions,my_position,dist,items,prev,enemies)
            # print(data.shape)
            data = xgb.DMatrix(data)
            pred = bst.predict(data)
            train_predictions = [round(value) for value in pred]
            act = int(train_predictions[0])
            # print(act)
